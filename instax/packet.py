"""Fujifilm Instax SP-x Packet Library.

This packet library can be used to encode and decode packets to be sent to
or recieved from a Fujifilm Instax SP-x. It is designed to be used with the
...
class VersionCommand(Packet):
    ...
    def __init__(self, mode, byteArray=None, unknown1=None, firmware=None, hardware=None):
        ...
        if byteArray is not None:
            self.byteArray = byteArray
            self.header = super(VersionCommand, self).decodeHeader(mode, byteArray)
            self.valid = self.validatePacket(byteArray, self.header["packetLength"])
            if mode == self.MESSAGE_MODE_COMMAND:
                self.decodedCommandPayload = self.decodeComPayload(byteArray)
            elif mode == self.MESSAGE_MODE_RESPONSE:
...
class PrintCountCommand(Packet):
    ...
    def __init__(self, mode, byteArray=None, printHistory=None):
        ...
        if byteArray is not None:
            self.byteArray = byteArray
            self.header = super(PrintCountCommand, self).decodeHeader(mode, byteArray)
            self.valid = self.validatePacket(byteArray, self.header["packetLength"])
            if mode == self.MESSAGE_MODE_COMMAND:
                self.decodedCommandPayload = self.decodeComPayload(byteArray)
            elif mode == self.MESSAGE_MODE_RESPONSE:
...
class ModelNameCommand(Packet):
    ...
    def __init__(self, mode, byteArray=None, modelName=None):
        ...
        if byteArray is not None:
            self.byteArray = byteArray
            self.header = super(ModelNameCommand, self).decodeHeader(mode, byteArray)
            self.valid = self.validatePacket(byteArray, self.header["packetLength"])
            if mode == self.MESSAGE_MODE_COMMAND:
                self.decodedCommandPayload = self.decodeComPayload(byteArray)
            elif mode == self.MESSAGE_MODE_RESPONSE:
...
class PrePrintCommand(Packet):
    ...
    def __init__(self, mode, byteArray=None, cmdNumber=None, respNumber=None):
        ...
        if byteArray is not None:
            self.byteArray = byteArray
            self.header = super(PrePrintCommand, self).decodeHeader(mode, byteArray)
            self.valid = self.validatePacket(byteArray, self.header["packetLength"])
            if mode == self.MESSAGE_MODE_COMMAND:
                self.decodedCommandPayload = self.decodeComPayload(byteArray)
            elif mode == self.MESSAGE_MODE_RESPONSE:
...
class PrinterLockCommand(Packet):
    ...
    def __init__(self, mode, lockState=None, byteArray=None):
        ...
        if byteArray is not None:
            self.byteArray = byteArray
            self.header = super(PrinterLockCommand, self).decodeHeader(mode, byteArray)
            self.valid = self.validatePacket(byteArray, self.header["packetLength"])
            if mode == self.MESSAGE_MODE_COMMAND:
                self.decodedCommandPayload = self.decodeComPayload(byteArray)
            elif mode == self.MESSAGE_MODE_RESPONSE:
...
class Type83Command(Packet):
    ...
    def __init__(self, mode, byteArray=None):
        ...
        if byteArray is not None:
            self.byteArray = byteArray
            self.header = super(Type83Command, self).decodeHeader(mode, byteArray)
            self.valid = self.validatePacket(byteArray, self.header["packetLength"])
            if mode == self.MESSAGE_MODE_COMMAND:
                self.decodedCommandPayload = self.decodeComPayload(byteArray)
            elif mode == self.MESSAGE_MODE_RESPONSE:
...
class Type195Command(Packet):
    ...
    def __init__(self, mode, byteArray=None):
        ...
        if byteArray is not None:
            self.byteArray = byteArray
            self.header = super(Type195Command, self).decodeHeader(mode, byteArray)
            self.valid = self.validatePacket(byteArray, self.header["packetLength"])
            if mode == self.MESSAGE_MODE_COMMAND:
                self.decodedCommandPayload = self.decodeComPayload(byteArray)
            elif mode == self.MESSAGE_MODE_RESPONSE:
...
class LockStateCommand(Packet):
    ...
    def __init__(self, mode, byteArray=None, unknownFourByteInt=None):
        ...
        if byteArray is not None:
            self.byteArray = byteArray
            self.header = super(LockStateCommand, self).decodeHeader(mode, byteArray)
            self.valid = self.validatePacket(byteArray, self.header["packetLength"])
            if mode == self.MESSAGE_MODE_COMMAND:
                self.decodedCommandPayload = self.decodeComPayload(byteArray)
            elif mode == self.MESSAGE_MODE_RESPONSE:
